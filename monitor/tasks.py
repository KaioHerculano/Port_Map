...
def dispatch_scheduled_checks() -> str:
    ...
    active_intervals = active_targets_query.values_list(
        "check_interval", flat=True
    ).distinct()
...
def send_monthly_telegram_report() -> str:
    ...
    targets = (
        ...
        .annotate(
            total_logs=Count(
                "logs",
                filter=Q(
                    logs__timestamp__gte=first_day_prev_month,
                    logs__timestamp__lte=last_day_prev_month,
                ),
            ),
            success_logs=Count(
                "logs",
        ...
        .annotate(
            availability=Case(
                When(
                    total_logs=0,
                    then=Case(
                        When(last_status=True, then=Value(100.0)), default=Value(0.0)
                    ),
                ),
                default=Cast(
    ...
    if success:
        return (
            f"Monthly report sent successfully. Avg availability: {avg_availability}%"
        )
    else:
...
def aggregate_daily_logs() -> str:
    ...
    from .models import DailySummary, MonitorLog, MonitorTarget
    ...
    for target in targets:
        logs = MonitorLog.objects.filter(
            target=target, timestamp__range=(start_time, end_time)
        )
        ...
        DailySummary.objects.update_or_create(
            target=target,
            date=yesterday,
            defaults={"availability": avail, "avg_latency": avg_lat},
        )
        created_count += 1
